...
OUT = Path('/root/workspace/outputs/arrester/stage2/patch_inpaint_preview_v4')
...
for i,job in enumerate(jobs):
    ...
    out=pipe(prompt=PROMPT, negative_prompt=NEGATIVE, image=comp_fit, mask_image=mask_fit,
             height=comp_fit.size[1], width=comp_fit.size[0],
    ...
    comp_np=np.array(comp_fit.convert('RGB'))
    mask_np=np.array(mask_fit.convert('L')).astype(np.float32) / 255.0
    # Only take SD2 pixels inside the inpaint mask. Keep the rest of the composite/original patch.
    # This avoids pasting a visible rectangular generated patch back into the full context.
    mask_blur=cv2.GaussianBlur(mask_np, (0,0), 2.0)
    blended_patch=(out_np.astype(np.float32)*mask_blur[:,:,None] + comp_np.astype(np.float32)*(1-mask_blur[:,:,None])).astype(np.uint8)
    full=context.copy()
    full[y1p:y1p+ph, x1p:x1p+pw]=blended_patch
    ...
    Image.fromarray(blended_patch).save(inp_path,quality=95)
    ...
    records.append({**job,'patch_box_xyxy':patch_box,'target_local_xyxy':target_local,'pasted_box_local_xyxy':pasted_box,'pasted_box_global_xyxy':pasted_global,'patch_composite':str(comp_path),'patch_inpainted_blended':str(inp_path),'full_context_inpainted':str(full_path),'overlay':str(overlay_path),'inpaint_mask':str(mask_path),'compare':str(compare_path),'strength':STRENGTH,'steps':STEPS,'guidance_scale':GUIDANCE,'paste_policy':'prototype_tight_cutout_height_matched_preserve_aspect_patch_only_inpaint_mask_blended_back' , 'blend_policy':'only SD2 pixels under inpaint mask are blended back; no rectangular patch paste'})
...
(OUT/'v4_manifest.json').write_text(json.dumps({'count':len(records),'records':records},ensure_ascii=False,indent=2))
...
sheet_path=OUT/'v4_contact_sheet.jpg'; sheet.save(sheet_path,quality=92)
print(json.dumps({'out':str(OUT),'count':len(records),'contact_sheet':str(sheet_path),'manifest':str(OUT/'v4_manifest.json')},ensure_ascii=False,indent=2))
